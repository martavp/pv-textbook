...
EQE_sample=load('C:\Pilar\Caltech_Projects\LIBRO\Problems\EQE_BC_EOL.txt');

%%%%%%%%%%CG%%%%%%%%%%%%%
Trans_CG=load('C:\Pilar\Caltech_Projects\LIBRO\Problems\TransmissionCoverGlass_BOL.txt');
T_CG= interp1 (Trans_CG(:,1), Trans_CG(:,2),EQE_sample(:,1),'linear','extrap');
T_losses_EOL=load('C:\Pilar\Caltech_Projects\LIBRO\Problems\T_losses_EOL.txt');
T_losses=interp1 (T_losses_EOL(:,1), T_losses_EOL(:,2),EQE_sample(:,1),'linear','extrap');

for i=1:length(EQE_sample(:,1))
    T_glass_EOL(i,1)= T_CG(i,1).*T_losses(i,1);
    EQE_sample(i,2)=T_glass_EOL(i,1).*EQE_sample(i,2);
end
data_EQE(:,1)=EQE_sample(:,1);
data_EQE(:,2)=EQE_sample(:,2);

figure(1)
plot(EQE_sample(:,1), T_CG(:,1),'*b');
hold on 
plot(EQE_sample(:,1), T_glass_EOL(:,1),'*r');
hold off
...
plot(EQE_sample(:,1), T_CG(:,1),'b*')
figure(3)
plot(EQE_sample(:,1),EQE_sample(:,2), 'b*')
