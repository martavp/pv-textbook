...
iam_Martin_Ruiz_soiled = iam.martin_ruiz(aoi, a_r=0.21)

plt.plot(aoi, iam_physical, '.', label='Detailed physical model (clean)')
plt.plot(aoi, iam_ASHRAE, label='ASHRAE model (clean)')
plt.plot(aoi, iam_Martin_Ruiz, label='Martin-Ruiz model (clean)')
plt.plot(aoi, iam_Martin_Ruiz_soiled, '--', label='Martin-Ruiz model (medium soiled)')
