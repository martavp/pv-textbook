...
cutout = atlite.Cutout(
    path="Denmark-2020-06", module="era5", bounds=Denmark.unary_union.bounds, #bounds=Iberia.unary_union.bounds, 
    time="2020-06")
cutout.prepare()
...
#%%
cap_factors = 0.5*cutout.pv(panel="CSi", 
...
Denmark.plot(ax=ax, zorder=1, transform=plate(), alpha=0.3)
cells.plot(ax=ax, **plot_grid_dict)
...
power_generation = (1/3)*(cutout.pv(panel="CSi", orientation={"slope": 30.0, "azimuth": 180.0},
                             layout=layout.fillna(0), shapes=Denmark) #shapes=UkIr)
                             .to_pandas()
