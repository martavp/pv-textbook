...
data=pd.DataFrame(data=None, 
                  index=None, 
                  columns=['lon', 'lat', 'distance','ro_wind', 'ro_radiation'], 
                  dtype=float)
...
k=0
i_min=max(0,index_lon-40)
i_max=min(len(nc.variables['lon'][:].data),index_lon+40)
j_min=max(0,index_lat-40)
j_max=min(len(nc.variables['lat'][:].data),index_lat+40)
for i in np.arange(i_min,i_max): #np.arange(0,len(nc.variables['lon'][:].data)):
    for j in np.arange(j_min,j_max): #np.arange(0,len(nc.variables['lat'][:].data)):
        k=k+1
        ...
        data.loc[k,'ro_wind'] = np.corrcoef(wind,wind_ref)[0,1]
        data.loc[k,'ro_radiation'] = np.corrcoef(radiation,radiation_ref)[0,1]
        
#%%

data.to_csv('data_correlation.csv', sep=',')
data2=pd.read_csv('data_correlation.csv', sep=',', index_col=0)
...
ax0.plot(data2['distance'],data2['ro_wind'],
         marker='.', 
         markersize=2,
         linewidth=0,
         color='dodgerblue')
...
ax1.plot(data2['distance'],data2['ro_radiation'],
         marker='.', 
         markersize=1,
         linewidth=0,
         color='orange')
...
#ax1.plot(dis,cor_theo_radiation, 'gray', linewidth=3)
