...
tmy_file = Path('data/annual_irradiation_tmy.csv')
...
ax_polar.set_rgrids(rtick_locs, labels=rtick_labels, fontsize=14)
...
for theta in thetatick_locs:
    if theta <= 180:
        ...
        thetatick_labels.append(f'{180 - theta}°')
                                     
ax_polar.set_thetagrids(thetatick_locs, labels=thetatick_labels, fontsize=14)
ax_polar.xaxis.set_tick_params(pad=7)
ax_polar.set_xlabel('Orientation [°]')

# tilt
ax_polar.set_ylabel('Tilt [°]', rotation=0)
ax_polar.yaxis.set_label_coords(0.81, 1.01)
ax_polar.set_rmax(90)
ax_polar.set_rlabel_position(158)

# [left, bottom, width, height]
cbar_ax = fig.add_axes([0.55, 0.25, 0.015, 0.55])
spec_cbar = fig.add_gridspec(nrows=1, ncols=1, left=0.55, right=0.57)
# cbar_ax = fig.add_subplot(spec_cbar[0, 0])
...
# add cubes
imagebox_0_0 = OffsetImage(plt.imread("data/annual_irradiation - cube_0_0.png"), zoom=0.15)
imagebox_S_opt = OffsetImage(plt.imread("data/annual_irradiation - cube_S_opt.png"), zoom=0.15)
imagebox_S_90 = OffsetImage(plt.imread("data/annual_irradiation - cube_S_90.png"), zoom=0.15)
imagebox_SE_opt = OffsetImage(plt.imread("data/annual_irradiation - cube_SE_opt.png"), zoom=0.15)
imagebox_SE_90 = OffsetImage(plt.imread("data/annual_irradiation - cube_SE_90.png"), zoom=0.15)
imagebox_E_opt = OffsetImage(plt.imread("data/annual_irradiation - cube_E_opt.png"), zoom=0.15)
imagebox_E_90 = OffsetImage(plt.imread("data/annual_irradiation - cube_E_90.png"), zoom=0.15)
...
ax_polar.add_artist(AnnotationBbox(imagebox_S_90, xy=(np.radians(0), 90-5), frameon=False))
ax_polar.add_artist(AnnotationBbox(imagebox_SE_opt, xy=(np.radians(45), tilt_range[idx_max[1]]), frameon=False))
ax_polar.add_artist(AnnotationBbox(imagebox_SE_90, xy=(np.radians(45), 90-5), frameon=False))
ax_polar.add_artist(AnnotationBbox(imagebox_E_opt, xy=(np.radians(90), tilt_range[idx_max[1]]), frameon=False))
ax_polar.add_artist(AnnotationBbox(imagebox_E_90, xy=(np.radians(90), 90-5), frameon=False))

spec_cube = fig.add_gridspec(nrows=1, ncols=1, left=0.7)
ax_cube = fig.add_subplot(spec_cube[0,0])
ax_cube.imshow(plt.imread('data/annual_irradiation - cube_main.png'))
ax_cube.axis('off')

plt.savefig('annual_irradiation.png', dpi=300)