...
series_calc_file = Path('data/tmy_generation_seriescalc.csv')
...
tmy_file = Path('data/tmy_generation_year.csv')
...
for num_month, year_series_calc in enumerate(unique_years):
    ...
    for month_tmy_year in [month for month, year in tmy_months.items() if year == year_series_calc]:
        ...
        ax_year.text(x=0.18, y=0.05, s=year_series_calc, color='white', transform=ax_year.transAxes)
        ...
        ax_tmy.text(x=30.417*(month_tmy_year-1)+7, y=1, s=dt.date(year=2010, month=month_tmy_year, day=1).strftime('%b'), color='white')
...
ax_legend.set_xlabel('Day of the year', fontsize=10)
ax_legend.set_ylabel('Hour of the day', fontsize=10)
ax_legend.tick_params(axis='x', labelsize=10)
ax_legend.tick_params(axis='y', labelsize=10)

cbar_ax = fig.add_axes([0.91, 0.115, 0.015, 0.765])
cb = fig.colorbar(im, cax=cbar_ax)
cb.set_label(label='Global Irradiance [$W·m^{-2}]$', size=12)
cb.ax.tick_params(labelsize=12)

plt.savefig('tmy_generation.png', bbox_inches='tight', dpi=300)