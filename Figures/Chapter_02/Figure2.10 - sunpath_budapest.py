...
def plot_cartesian(ax, location, times):
    ...
    for hour in np.unique(solpos.index.hour):
        ...
        if hour > 12:
            ...
            ha = 'left'
        ax.text(pos['azimuth'], pos['apparent_elevation'], str(hour)+'h', size=8, horizontalalignment=ha) # hour in summer includes DST
    ...
    ax.set_ylabel('Solar Elevation [°]', fontsize=14)
    ax.set_xlabel('Solar Azimuth [°]', fontsize=14)
        
    ax.xaxis.set_major_locator(FixedLocator(np.arange(start=0, stop=360, step=45)))
    ax.xaxis.set_major_formatter(FixedFormatter(np.arange(start=0, stop=360, step=45) - 180))
    ...
    ax.set_ylim([0, 90])
    ax.yaxis.set_ticks(np.arange(0, 90, 15))
...
def plot_polar(ax, location, times):
    ...
    for hour in np.unique(solpos.index.hour):
        ...
        if hour < 12:
            ...
            ha = 'left'
        ax.text(np.radians(180 - pos['azimuth']), 90 - pos['apparent_elevation'], str(hour)+'h', size=8, horizontalalignment=ha) # hour in summer includes DST
    ...
    ax.set_rgrids(radii=rtick_locs, labels=rtick_labels, fontsize=10)
    ...
    ax.set_ylabel('Solar Elevation [°]', fontsize=14, rotation=0)
    ax.yaxis.set_label_coords(0.85, 1.01)
    ax.set_rmax(90)
    ax.set_rlabel_position(158)
    
    ax.set_xlabel('Solar Azimuth [°]', fontsize=14)
        
    for letter, position in zip(['N', 'W', 'S', 'E'], [-180, -90, 0, 90]) :
        ax.annotate(letter, xy=(np.radians(position), 80),  xycoords='data', fontsize=16, horizontalalignment='center', verticalalignment='center')
...
plt.savefig('sunpath_budapest_polar.png', dpi=300)
...
plt.savefig('sunpath_budapest_cartesian.png', dpi=300)